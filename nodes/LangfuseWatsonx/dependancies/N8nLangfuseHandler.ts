...
export class N8nLangfuseHandler extends BaseCallbackHandler {
    ...
    async handleLLMStart(llm: Serialized, prompts: string[]) {
        ...
        this.input = typeof raw === 'string'
            ? raw.replace(/^Human:\s*/, '')
            : (raw as any)?.text || String(raw);

        const workflowSlug = slugifyWorkflowName(this.metadata.workflowName);
        this.logger.log("[WatsonX-Langfuse] Creating trace...");
        this.trace = this.langfuse.trace({ name: `langfuse-test-${workflowSlug}-trace`, userId: 'n8n-watsonx-llmx-test' });
        ...
        await this.trace.update({ input: this.input, metadata: { step: "started", startTime: this.startTime, ...this.metadata} });
        await this.span.update({ input: this.input, metadata: { step: "started", startTime: this.startTime, ...this.metadata} });
    }
